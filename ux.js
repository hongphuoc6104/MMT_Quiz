(() => {
  ...
  function renderQuestionPalette(){
    ...
    state.pool.forEach((question,index)=>{
      ...
      if(answer?.selected)btn.classList.add(answer.selected===question.correct_option_id?'done':'wrong');
      btn.onclick=()=>{
        state.idx=index;
        render();
        scrollToQuiz();
  ...
  solutionHtml=function(question,s,order){
    ...
    const optionRows=s.options.map(item=>{
      ...
      return `<article class="option-analysis ${item.correct?'is-correct':'is-wrong'}">
        <h5>${item.correct?'✅':'❌'} ${letter}. ${escapeHtml(item.text)}</h5>
        <p><b>Vì sao ${item.correct?'chọn':'không chọn'}:</b> ${escapeHtml(item.why)}</p>
